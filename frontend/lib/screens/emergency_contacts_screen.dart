...
class _EmergencyContactsScreenState extends State<EmergencyContactsScreen> {
  final EmergencyContactService service = EmergencyContactService();
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                final success = await service.addContact(name, phone, relation);
  ...
  Future<void> registerTestFcmToken(dynamic contact) async {
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text("FCM error: $e")));
    }
  }
  ...
  Future<void> linkContactUser(dynamic contact) async {
    ...
    final success = await service.linkContactUser(contact["id"], userId);
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          success ? "Contact linked successfully" : "Failed to link contact",
        ),
      ),
    );
      ...
      body: loading
          ...
          ? const Center(
              child: Column(
                ...
                children: [
                  Icon(Icons.contact_phone, size: 80, color: Colors.grey),
                  SizedBox(height: 16),
                  Text(
                  ...
                  SizedBox(height: 8),
                  Text("Tap + to add a trusted contact"),
                ],
              ),
            )
          : RefreshIndicator(
